r"""
================================================================================
GLO-NCA MAX-PUSH — all accuracy levers stacked (Kaggle, single cell)
================================================================================
Goal: push WT toward ~0.88-0.90 and TC/ET toward ~0.78-0.82. Combines:

  1. HIGH-RES 3-level cascade   : patches up to 96^3  -> recovers small ET/TC
  2. MORE EPOCHS + LR WARMUP    : 250 epochs, linear warmup then cosine decay
  3. MORE CAPACITY              : channel_n 32, hidden 128, steps 20
  4. PSEUDO-ENSEMBLE at test    : average 10 stochastic inferences (free gain)
  5. POST-PROCESSING            : keep largest connected component / drop specks
  6. STRONG AUGMENTATION        : flips + 90-deg rotations + intensity jitter
  + Tversky+BCE loss (beta>alpha) for ET/TC recall

WARNING: this is a LONG run (high-res + 250 epochs). On a Kaggle T4 expect
several hours. Use "Save & Run All (Commit)". Lower EPOCHS / patch size if the
12h limit is a risk. VRAM is higher here (~3-5 GB) but fits a T4 (15 GB).

USAGE (one Kaggle cell, GPU on, BraTS attached):
    !rm -rf GLO-NCA && git clone -q https://github.com/Muhammadwaqas1234/GLO-NCA.git
    %run GLO-NCA/kaggle_maxpush.py
================================================================================
"""
...
# ---- knobs ----
EPOCHS      = 250
WARMUP_EP   = 5            # linear LR warmup over the first WARMUP_EP epochs
N_PATIENTS  = None
SEED        = 42
CHANNEL_N   = 32
HIDDEN      = 128
STEPS       = [20, 20, 20]          # 3 levels
INPUT_SIZE  = [[24, 24, 16], [48, 48, 32], [96, 96, 64]]   # 3-level, high-res top
TRAIN_MODEL = 2                     # 2 down-scaling levels -> 3 NCAs
LR_START    = 16e-4
LR_MIN      = 1e-5
ENSEMBLE_N  = 10           # pseudo-ensemble inferences at test
POSTPROC    = True
REGIONS     = ["WT", "TC", "ET"]
...
OUT_DIR  = "/kaggle/working/glo_maxpush"
...
def augment(img, label):
    ...
    if random.random() < 0.5:                      # in-plane 90-deg rotation
        k = random.choice([1, 2, 3])
        img = np.rot90(img, k, axes=(0, 1)); label = np.rot90(label, k, axes=(0, 1))
    if random.random() < 0.5:                      # intensity jitter (per modality)
        img = img * np.float32(random.uniform(0.9, 1.1)) + np.float32(random.uniform(-0.05, 0.05))
...
# ---- (5) post-processing: keep largest connected component, drop small specks ----
def postprocess(mask, min_voxels=20):
    ...
    keep[lbl == largest] = 1
    for comp in range(1, n + 1):                   # also keep other sizable comps
        if sizes[comp - 1] >= min_voxels:
            keep[lbl == comp] = 1
    return keep


# ---- (4) pseudo-ensemble test eval + post-processing ----
    ...
    with torch.no_grad():
        for data in loader:
            ...
            for i, r in enumerate(REGIONS):
                ...
                if postproc:
                    p = postprocess(p[0])[None] if p.ndim == 4 else postprocess(p)
                t = gt[..., i]
                inter = np.logical_and(p >= 0.5, t >= 0.5).sum()
                acc[r]["dice"].append((2*inter)/((p >= 0.5).sum()+(t >= 0.5).sum()+1e-6))
                acc[r]["iou"].append(iou_score(p.astype(float), t)); acc[r]["hd95"].append(hd95_score(p.astype(float), t))
...
def main():
    ...
    print(f"Config: ch={CHANNEL_N} hidden={HIDDEN} steps={STEPS} levels={TRAIN_MODEL+1} "
          f"patch_top={INPUT_SIZE[-1]} epochs={EPOCHS} warmup={WARMUP_EP}", flush=True)
    ...
    kernels = [7, 3, 3]
    ca = [BasicNCA3D(CHANNEL_N, 0.5, dev, HIDDEN, kernel_size=kernels[i], input_channels=4, use_attention=True)
          for i in range(TRAIN_MODEL + 1)]
    ...
    # ---- (2) LR warmup + cosine via LambdaLR (per-batch stepping by base agent) ----
    ...
    def lr_lambda(step):
        ...
        prog = (step - warmup_steps) / max(1, total_steps - warmup_steps)   # cosine 1 -> floor
    ...
    loss_f = TverskyCELoss(alpha=0.3, beta=0.75, ce_weight=0.5)
    ...
    for ep in range(EPOCHS):
        ...
        for i, data in enumerate(torch.utils.data.DataLoader(ds, shuffle=True, batch_size=1)):
            ...
            print(f"\rep {ep+1}/{EPOCHS} training {i+1}/{steps_per_epoch}...", end="", flush=True)
        cur_lr = agent.optimizer[0].param_groups[0]["lr"]
        val = evaluate(agent, ds, "val")              # plain (fast) val
    ...
    # ---- final TEST: plain, then +ensemble, then +ensemble+postproc (show each gain) ----
    ck = torch.load(best_path, map_location=dev)
    for m, sd in zip(ca, ck["m"]):
        m.load_state_dict(sd)
    print(f"\nBest model @ epoch {ck['ep']} (val mean {ck['val_mean']:.3f}); evaluating on TEST...",
          flush=True)
    ...
    def show(lab, t):
        ...
        print(f"{lab:<28}{t['WT']['dice']:<8.3f}{t['TC']['dice']:<8.3f}{t['ET']['dice']:<8.3f}{m:<8.3f}")
    print("\n" + "=" * 64)
    print("GLO-NCA MAX-PUSH — FINAL TEST (Dice)")
    print("=" * 64)
    print(f"{'setting':<28}{'WT':<8}{'TC':<8}{'ET':<8}{'mean':<8}")
    print("-" * 64)
    ...
    json.dump({"plain": test_plain, "ensemble": test_ens, "ensemble_postproc": test_full,
              ...
              open(os.path.join(OUT_DIR, "maxpush_results.json"), "w"), indent=2, default=str)
    ...
    plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR, "maxpush_curves.png"), dpi=130); plt.show()
