...
def walk_active():
    ...
    audit_scripts = {SELF,
                     os.path.join(_ROOT, "scripts", "phase2_regression.py"),
...
def t_no_contradictory_production_docs():
    ...
    for rel in ("README.md", "cloud/README.md",
                "docs/thesis/FINAL_TRAINING_PROTOCOL.md"):
...
def t_v2_fallback_blocked():
    bad = []
    if 'default=os.path.join("configs", "gcp_full.yaml")' in read("train.py"):
...
def t_phase1_reports_intact():
    d = os.path.join(_ROOT, "reports", "audit")
...
def main():
    ...
    out = os.path.join(_ROOT, "reports", "validation", "phase1_closure_audit.json")
