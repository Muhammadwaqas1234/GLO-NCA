...
#
# Usage:
#   ./cloud/scripts/verify_results.sh <experiment_id>   # must PASS first
#   ./cloud/scripts/delete_vm.sh <experiment_id>        # verifies, then deletes
#   ./cloud/scripts/delete_vm.sh --force                # delete without verify
# =============================================================================
...
if [[ "${ARG}" == "--force" ]]; then
  ...
  if ! bash "${CLOUD_DIR}/scripts/verify_results.sh" "${EXP_ID}"; then
...
log "Sanity-check for stray costly resources with: ./cloud/scripts/cost_report.sh --audit"
