...
def t_production_config():
    import yaml
    cfg = yaml.safe_load(open(os.path.join(_ROOT, "extra/configs/historical/v3_multilevel_ckpt.yaml")))
    want = {
        ("experiment", "seed"): 42,
        ("training", "epochs"): 300,
...
def t_param_count():
    ...
    cfg = load_config(os.path.join(_ROOT, "extra/configs/historical/v3_multilevel_ckpt.yaml"))
...
def t_v3_forward_backward():
    ...
    cfg = load_config(os.path.join(_ROOT, "extra/configs/historical/smoke_test_v3.yaml"))
    m = build_v3_from_config(cfg, 4, 3, torch.device("cpu"))
...
def t_no_v2_production_default():
    ...
    if 'default=os.path.join("extra", "configs", "historical", "gcp_full.yaml")' in tp:
    ...
    rt = io.open(os.path.join(_ROOT, "cloud/scripts/run_training.sh"),
                 encoding="utf-8").read()
    if '${1:-extra/configs/historical/gcp_full.yaml}' in rt:
