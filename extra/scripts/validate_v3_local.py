...
def core_checks(device):
    ...
    cfg = load_config(os.path.join(_REPO, "extra", "configs", "historical", "smoke_test_v3.yaml"))
    ...
    v2_ok = all(os.path.exists(os.path.join(_REPO, f)) for f in
                ["extra/configs/historical/gcp_full.yaml", "src/models/Model_GLO_NCA_Cell.py"])
...
def main():
    ...
    gpu_memory_ladder(os.path.join(_REPO, "extra", "configs", "historical", "v3_multilevel.yaml"))
