...
def hd95_score(pred, target, threshold=0.5):
    ...
    if pred_bin.sum() == 0 or target_bin.sum() == 0:
        return float('nan')
    d_pred_to_gt = _surface_distances(pred_bin, target_bin)
    d_gt_to_pred = _surface_distances(target_bin, pred_bin)
    ...
    def test(self, loss_f, save_img = None, tag='test/img/', pseudo_ensemble=False, showResults=False, **kwargs):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader):
                ...
                # --------------------------------- 3D ----------------------------
                    ...
                    for m in range(patient_3d_image.shape[-1]):
                        ...
                        hd95_log[m][patient_id] = hd95_score(pred_prob, gt_np)
            ...
            for key in loss_log.keys():
                ...
                if len(iou_log[key]) > 0:
                    print("Average mIoU 3d: " + str(key) + ", " + str(sum(iou_log[key].values())/len(iou_log[key])))
                # HD95 averaged only over patients where it is defined (not NaN).
                valid_hd = [v for v in hd95_log[key].values() if not math.isnan(v)]
                if len(valid_hd) > 0:
