...
def main():
    ...
    print(f"v5: ch={CHANNEL_N} hidden={HIDDEN} steps={STEPS} fire={FIRE_RATE} patch={INPUT_SIZE[-1]} "
    ...
    print("\n" + "=" * 60)
    print(f"GLO-NCA v5 — FINAL TEST (best @ epoch {ck['ep']})")
    print("=" * 60)
    ...
    json.dump({"test": test, "history": hist, "best_epoch": ck["ep"], "params": n_params,
              ...
              open(os.path.join(OUT_DIR, "v5_results.json"), "w"), indent=2, default=str)
    ...
    a2.set_title("Validation Dice (v5)"); a2.set_ylim(0, 1); a2.legend(); a2.grid(alpha=.3)
    plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR, "v5_curves.png"), dpi=130); plt.show()
