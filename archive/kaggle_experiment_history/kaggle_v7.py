r"""
================================================================================
GLO-NCA v7 — v6 + augmentation + FocalTversky + EMA + grad-clip + dropout (anti-overfit for 882 cases)
================================================================================
...
SEED        = 42
CHANNEL_N   = 24
HIDDEN      = 128            # v4 was 96 (matches DENSE-128 in slides)
USE_SPATIAL = True           # v5: add the spatial global-context block
PRIORITIZE_REGION = 2        # v6: bias patches toward ET (2) - the rarest region
ENSEMBLE_N  = 10             # v6: pseudo-ensemble at final test
USE_TTA     = True           # v6: test-time augmentation (flips) at final test
...
TVERSKY_BETA = 0.75
FOCAL_GAMMA  = 1.33          # v7: Focal Tversky exponent (focus on hard ET)
DROPOUT      = 0.1           # v7: light dropout in NCA MLP
USE_EMA      = True          # v7: exponential moving average of weights
EMA_DECAY    = 0.999
GRAD_CLIP    = 1.0           # v7: gradient clipping

# ---- preprocessing (keep v3's wins) ----
USE_FOREGROUND_CROP = True
USE_NONZERO_NORM    = True
USE_AUG             = True     # v7: ON (safe now with 882 cases; anti-overfit)    # OFF in v4 (was the convergence drag in v3)
...
from src.losses.LossFunctions import TverskyCELoss, FocalTverskyCELoss
...
def evaluate(agent, dataset, state, ensemble=1, tta=False):
    """Evaluate with optional test-time boosters (used only for the final test):
        ensemble>1 : average N stochastic NCA inferences (pseudo-ensemble)
        tta=True   : also average over 3-axis flips (test-time augmentation)
    Fast val uses ensemble=1, tta=False (defaults).
    """
...
def main():
    ...
    print(f"v7: ch={CHANNEL_N} hidden={HIDDEN} steps={STEPS} fire={FIRE_RATE} patch={INPUT_SIZE[-1]} "
    ...
    config = [{
        ...
        "patchify": True, "priotize_masks": 0.7, "prioritize_region": PRIORITIZE_REGION,
    }]
    ds = BraTS_FG(); ds.MODALITIES = MODALITIES
    dev = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ca = [BasicNCA3D(CHANNEL_N, FIRE_RATE, dev, HIDDEN, kernel_size=7, input_channels=4, use_attention=True, use_spatial=USE_SPATIAL, dropout=DROPOUT),
          BasicNCA3D(CHANNEL_N, FIRE_RATE, dev, HIDDEN, kernel_size=3, input_channels=4, use_attention=True, use_spatial=USE_SPATIAL, dropout=DROPOUT)]
    ...
    # --- gradient clipping via backward hooks (batch_step steps the optimizer) ---
    if GRAD_CLIP and GRAD_CLIP > 0:
        for m in ca:
            for p in m.parameters():
                if p.requires_grad:
                    p.register_hook(lambda g: torch.clamp(g, -GRAD_CLIP, GRAD_CLIP))
    ...
    for ep in (range(EPOCHS) if not REEVAL_ONLY else []):
        ...
        for i, data in enumerate(loader):
            ...
            ema_update()                       # v7: update EMA shadow weights
            if r:
    ...
    # Final test: plain, then with pseudo-ensemble + TTA (show each gain).
    test_plain = evaluate(agent, ds, "test", ensemble=1, tta=False)
    test = evaluate(agent, ds, "test", ensemble=ENSEMBLE_N, tta=USE_TTA)

    def show(lab, t):
        m = np.mean([t[r]['dice'] for r in REGIONS])
        print(f"{lab:<24}{t['WT']['dice']:<8.3f}{t['TC']['dice']:<8.3f}{t['ET']['dice']:<8.3f}{m:<8.3f}")
    print("\n" + "=" * 60)
    print(f"GLO-NCA v7 — FINAL TEST (best @ epoch {ck['ep']})")
    print("=" * 60)
    print(f"{'setting':<24}{'WT':<8}{'TC':<8}{'ET':<8}{'mean':<8}")
    print("-" * 60)
    show("plain", test_plain)
    show(f"+ensemble x{ENSEMBLE_N} +TTA" if USE_TTA else f"+ensemble x{ENSEMBLE_N}", test)
    print("-" * 60)
    ...
    json.dump({"test": test, "history": hist, "best_epoch": ck["ep"], "params": n_params,
              ...
              open(os.path.join(OUT_DIR, "v7_results.json"), "w"), indent=2, default=str)
    ...
    a2.set_title("Validation Dice (v7)"); a2.set_ylim(0, 1); a2.legend(); a2.grid(alpha=.3)
    plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR, "v7_curves.png"), dpi=130); plt.show()
