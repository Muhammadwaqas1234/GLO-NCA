r"""
================================================================================
GLO-NCA v6 — v5 + ET-aware sampling + test-time ensemble/TTA (for 882 cases)
================================================================================
...
def main():
    ...
    print(f"v6: ch={CHANNEL_N} hidden={HIDDEN} steps={STEPS} fire={FIRE_RATE} patch={INPUT_SIZE[-1]} "
    ...
    ca = [BasicNCA3D(CHANNEL_N, FIRE_RATE, dev, HIDDEN, kernel_size=7, input_channels=4, use_attention=True, use_spatial=USE_SPATIAL),
          BasicNCA3D(CHANNEL_N, FIRE_RATE, dev, HIDDEN, kernel_size=3, input_channels=4, use_attention=True, use_spatial=USE_SPATIAL)]
    ...
    print("\n" + "=" * 60)
    print(f"GLO-NCA v6 — FINAL TEST (best @ epoch {ck['ep']})")
    print("=" * 60)
    ...
    json.dump({"test": test, "history": hist, "best_epoch": ck["ep"], "params": n_params,
              ...
              open(os.path.join(OUT_DIR, "v6_results.json"), "w"), indent=2, default=str)
    ...
    a2.set_title("Validation Dice (v6)"); a2.set_ylim(0, 1); a2.legend(); a2.grid(alpha=.3)
    plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR, "v6_curves.png"), dpi=130); plt.show()
